...
modelinput = "DIEP\\ML\\Model\\"
TestDatainput = "DIEP\\ML\\Testset\\"
output = "DIEP\\results\\"
...
cvfile = pd.read_table(modelinput + "Test_Results.txt")
# negative
...
# test
DIDA_NDIt = pd.read_table(input + "Test_DIDA_NDI_FV.txt", na_values=".")
Randomt = pd.read_table(input + "Test_Random_FV.txt", na_values=".")
Alltest = pd.concat([DIDA_NDIt, Randomt], axis=0, ignore_index=True)

# load the model from disk
ModelNum = [2, 6, 10, 12, 15, 36, 55, 58, 65, 69, 77, 79, 87, 106, 109, 117, 120, 127, 145, 149, 160, 178, 179, 181, 182, 199]
ModelIndex = [x-1 for x in ModelNum]
ModelList = []
for i in ModelIndex:
    modelname = modelinput + "RF_finalized_model_" + str(i + 1) + ".mdl"
    model = pickle.load(open(modelname, 'rb'))
    ModelList.append(model)

# assign weight by 10x cross validation F1
...
# Predict on test set
Testset = pd.read_table(input + "xxxxxxcom_FV.txt", na_values=".")
